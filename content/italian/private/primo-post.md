---
title: Post con Utilities
translationKey: utilities-post
date: 2026-07-22T14:00:00
draft: false
description: Descrizione per SEO e anteprime social
summary: Testo che appare nelle card e nelle liste
cover: https://res.cloudinary.com/nhuws9fe/image/upload/v1783378859/Shrek_vtjnqb.png
---

Sono elencate tutte le funzionalità che il sito dispone

---

Annotazione {{< annotation >}} Questo e' un esempio di annotazione
{{< /annotation >}}

**Font da utilizzare:**

1. {{< font name="giocoso" size="2rem" >}} Giocoso {{< /font >}}
2. {{< font name="streghetta" size="2rem" >}} Streghetta {{< /font >}}
3. {{< font name="happy" size="2rem" >}} Happy {{< /font >}} 
4. {{< font name="paura" size="2rem" >}} Paura {{< /font >}}
5. {{< font name="pensieri" size="2rem" >}} Pensieri {{< /font >}}
6. {{< font name="murales" size="2rem" >}} Murales {{< /font >}}
7. {{< font name="psx" size="2rem" >}} Psx {{< /font >}}
8. {{< font name="stilizzato" size="2rem" >}} Stilizzato {{< /font >}}
9. {{< font name="fumetto" size="2rem" >}} Fumetto {{< /font >}}

**Inserire una immagine**
![Gumball foto di prova](https://res.cloudinary.com/nhuws9fe/image/upload/v1783376711/GumballTest_bitivy.jpg)

**Inserire una canzone di Spotify**
{{< spotify type="track" id="0vmFuEhyHR8SbOhDFq021Y?si=cc1e323f0d764a87" >}}

**Inserire una canzone da Navidrome**
{{< navidrome url="https://music.tommasofrancescon.it/share/6aNzLMAS0f" >}}

**Inserire una canzone da Navidrome che parte ad un tempo prestabilito**
{{< navidrome url="https://music.tommasofrancescon.it/share/6aNzLMAS0f" start="1:03" >}}

**Inserire un link**

[Sito di Amazon per Prova](https://www.amazon.it/)

**Inserire un video di Youtube**
{{< youtube O_IViNjCApU >}}

**Esempio per linee di codice**
{{< highlight go "linenos=true,hl_lines=3 5" >}}
package main
func main() {
    println("ciao")
}
{{< /highlight >}}

**Google Maps con URL totale**
{{< maps url="https://www.google.com/maps/place/Via+Candiolo,+26,+10127+Torino+TO/@45.0147227,7.6528157,17z/" >}}

**Google Maps con URL parziale**
{{< maps url="https://www.google.com/maps/@45.0145544,7.6532388,17z" >}}

**Google Maps con percorso. Partenza e Arrivo**
{{< maps url="https://www.google.com/maps/@45.0147227,7.6528157,17z/" to="https://www.google.com/maps/@45.0690511,7.6919054,16.83z/" >}}

**Inserire post di Instagram**
{{< instagram id=DZsBp0LjcFd hideCaption=true >}}

